...
const registerUser = asyncHandler(async (req, res) => {
    ...
    // Extracting the data from Front-End
    ...
    // checking for local path of image and avatar
    ...
    if (!avatarLocalPath) {
        //checking for avatar's local path as it is a required field in databaste
    ...
    if (!avatar) {
        //Again checking for avatar upload on cloudinary as it is a required field in databaste
        throw new ApiError(400, "Avatar file is required");
    }
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    // find user exists
    // password check
    // if password correct, successful login and give tokens
    // tokens will be sent through cookies(secure cookies)
...
const logoutUser = asyncHandler(async (req, res) => {
    await User.findByIdAndUpdate(
        req.user._id,
        {
            $unset: {// Use $unset to completely remove the field
                refreshToken: 1,
            },
            // $set: {
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    ...
    //console.log("\n\nRequest.body in refreshAccessToken: \n", req.body, "\n\n");
...
const updateUserAvatarImage = asyncHandler(async (req, res) => {
    ...
    if (!avatar.url) {
        throw new ApiError(
            400,
            "Error while uploading avatar image to cloudinary"
        );
    }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    console.log("Length: ",channel.length)
    if (!channel?.length) {
